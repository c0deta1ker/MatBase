...
[MatBase](https://github.com/c0deta1ker/MatBase) is an app that allows you to explore, analyze and extract the properties and photoelectron spectroscopy behaviours of various materials within MATLAB. You can use this toolbox of functions and Apps to:  
- Access, edit and add to a comprehensive database of material properties and photoelectron spectroscopy parameters within a MATLAB environment.  
- Visualize and manipulate crystal structures in both real and reciprocal space, and calculate the Brillouin zone and 2D slices through it.  
- Compute the electron inelastic mean free path (IMFP) for different materials using various formalisms, such as the Universal, NIST (Optical Experiments), TPP-2M, S1, S2, S3, S4, and JTP methods.  
...
- Simulate an N-layer sample stack composed of various layers, materials, thicknesses and determine the spectral intensity contribution from each layer, for a particular core level, and compare it to experimental data.    
