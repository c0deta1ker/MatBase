...
%       - ORC-oF-D2h   : Face-Centered Orthorhombic
%       - MCL-mP-C2h   : Simple Monoclinic
%       - MCL-mS-C2h   : Base-Centered Monoclinic
%       - TRI-aP-Ci    : Simple Triclinic
%
%   IN:
%   - crystal_args:   1x3 cell of {Type, (a, b, c), (alpha, beta, gamma)} described below;
%           -- Type;                "CUB-cP-Oh",  "CUB-cI-Oh", "CUB-cF-Oh", 
...
%   	.(crys_params): array of lattice parameters: [a, b, c; alpha, beta, gamma]
...
% --- If only the crystal structure is defined, default to unit parameters
if length(crystal_args) == 1 && isstring(crystal_args)
    crystal     = crystal_args{1};
    a = 1; b = a; c = a;
    alpha = 90; beta = alpha; gamma = alpha;
% --- If all crystal arguments are not defined, show an error
elseif length(crystal_args) ~= 3
    error('Crystal_args is not a {1x3} cell - check all input arguments are entered.');
% --- If all crystal arguments are defined, proceed to their definitions
else
    crystal     = crystal_args{1};
    % -- In units of Angstroms
    if isempty(crystal_args{3})
        a = 1; b = a; c = a;
    elseif length(crystal_args{3}) == 3
        a = crystal_args{2}(1);
        b = crystal_args{2}(2);
        c = crystal_args{2}(3);
    else
        error('No definition for the lattice parameters (a, b, c).');
    end
    % -- In units of degrees
    if isempty(crystal_args{3})
        alpha = 90; beta = alpha; gamma = alpha;
    elseif length(crystal_args{3}) == 3
        alpha       = crystal_args{3}(1);
        beta        = crystal_args{3}(2);
        gamma       = crystal_args{3}(3);
    else
        error('No definition for the lattice parameters (alpha, beta, gamma).');
    end
end
% -- Validity check on the input crystal structure
list_of_crystals =  ["CUB-cP-Oh", "CUB-cI-Oh", "CUB-cF-Oh",...
                        "HEX-hP-D6h", "RHL-hR-D3d",...
                        "TET-tP-D4h", "TET-tI-D4h",...
                        "ORC-oP-D2h", "ORC-oS-D2h", "ORC-oI-D2h", "ORC-oF-D2h",...
                        "MCL-mP-C2h", "MCL-mS-C2h",...
                        "TRI-aP-Ci"];
log_indx = strcmpi(crystal, list_of_crystals);
if sum(log_indx) == 1
    [~, idx] = max(log_indx); crystal = list_of_crystals(idx);
elseif contains(crystal, list_of_crystals)
    for i = 1:length(list_of_crystals); if contains(crystal, list_of_crystals(i)); idx = i; end; end
    crystal = list_of_crystals(idx);
else
    error('Crystal type is not properly defined. Should be one of: CUB-cP-Oh, CUB-cI-Oh, CUB-cF-Oh, HEX-hP-D6h, RHL-hR-D3d, TET-tP-D4h, TET-tI-D4h, ORC-oP-D2h, ORC-oS-D2h, ORC-oI-D2h, ORC-oF-D2h, MCL-mP-C2h, MCL-mS-C2h or TRI-aP-Ci.');
end
% -- Applying symmetry rules to the crystal arguments
% For 'CUB', 'BCC' and 'FCC' crystal
if crystal == "CUB-cP-Oh" || crystal == "CUB-cI-Oh" || crystal == "CUB-cF-Oh"
    % - Applying condition that alpha must be 90degrees and equal to beta and gamma
    alpha = 90; beta = alpha; gamma = alpha; 
% For 'HEX' crystal
elseif crystal == "HEX-hP-D6h"
    % - Applying condition that one angle must be 120deg, and the other two must be 90degrees
    if round(alpha,1) == 120; beta = 90; gamma = beta;
    elseif round(beta,1) == 120; alpha = 90; gamma = alpha;
    elseif round(gamma,1) == 120; alpha = 90; beta = alpha;
    else; alpha = 90; beta = alpha; gamma = 120;
    end
% For 'RHL' crystal
elseif crystal == "RHL-hR-D3d"
    % - Applying condition that alpha must not be 90degrees, but equal to beta and gamma
    if round(alpha,1) == 90; alpha = alpha - 1; beta = alpha; gamma = alpha;
    elseif round(beta,1) == 90; beta = beta - 1; alpha = beta; gamma = beta;
    elseif round(gamma,1) == 90; gamma = gamma - 1; alpha = gamma; beta = gamma;
    else; alpha = 89; beta = alpha; gamma = alpha;
    end
% For 'TET' and 'BCT' crystal
elseif crystal == "TET-tP-D4h" || crystal == "TET-tI-D4h"
    % - Applying condition that alpha must be 90degrees and equal to beta and gamma
    alpha = 90; beta = alpha; gamma = alpha; 
% For 'ORC', 'ORCC', 'ORCI' and 'ORCF' crystal
elseif crystal == "ORC-oP-D2h" || crystal == "ORC-oS-D2h" || crystal == "ORC-oI-D2h" || crystal == "ORC-oF-D2h"
    % - Applying condition that alpha must be 90degrees and equal to beta and gamma
    alpha = 90; beta = alpha; gamma = alpha;
% For 'MCL' and 'MCLC' crystal
elseif crystal == "MCL-mP-C2h" || crystal == "MCL-mS-C2h"
    % - Applying condition that one angle must be arbitrary, and the other two must be 90degrees
    if round(alpha,1) ~= 90; beta = 90; gamma = beta;
    elseif round(beta,1) ~= 90; alpha = 90; gamma = alpha;
    elseif round(gamma,1) ~= 90; alpha = 90; beta = alpha;
    else;  beta = 89; alpha = 90; gamma = beta;
    end
% For 'TRI' crystal
elseif crystal == "TRI-aP-Ci"
...
if crystal == "CUB-cP-Oh" || crystal == "HEX-hP-D6h" || crystal == "RHL-hR-D3d" ||...
        crystal == "TET-tP-D4h" || crystal == "ORC-oP-D2h" || crystal == "MCL-mP-C2h" ||...
        crystal == "TRI-aP-Ci" || crystal == "ORC-oS-D2h" || crystal == "MCL-mS-C2h"
...
elseif crystal == "CUB-cI-Oh" || crystal == "TET-tI-D4h" || crystal == "ORC-oI-D2h" ||...
        crystal == "CUB-cF-Oh" || crystal == "ORC-oF-D2h"
    G1 = 2*(2*pi*cross(T2,T3)/Tvol);
    G2 = 2*(2*pi*cross(T3,T1)/Tvol);
    G3 = 2*(2*pi*cross(T1,T2)/Tvol);
end
...
realStr.crys_params = [a,b,c;round(alpha*180/pi,1),round(beta*180/pi,1),round(gamma*180/pi,1)];
realStr.T1 = T1;
realStr.T2 = T2;
realStr.T3 = T3;
realStr.Tb0 = Tb0;
...
reciStr.crys_params = [a,b,c;round(alpha*180/pi,1),round(beta*180/pi,1),round(gamma*180/pi,1)];
reciStr.G1 = G1;
reciStr.G2 = G2;
reciStr.G3 = G3;
reciStr.Gb0 = Gb0;
