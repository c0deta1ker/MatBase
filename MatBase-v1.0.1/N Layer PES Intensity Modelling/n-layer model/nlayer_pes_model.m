function pes_model = nlayer_pes_model(lyr_mat, lyr_cls, lyr_thick, hv, theta, gamma, plot_result)
...
%   -   lyr_cls:  	Mx1 cell-vector of the core-level probed for each layer in the stack; e.g. "Si2p", "Si2s", "Al2p"...
%   -   lyr_thick:	Mx1 cell-vector of the thickness of each layer in the stack (in nano-metres)
%   -   hv:         scalar or Nx1 row vector of the input photon energy.
%   -   theta:      scalar or 1xM column of the angle between the emitted photoelectrons and the surface normal (degrees).
...
if nargin < 6; gamma = 81;  end
if nargin < 5; theta = 0;  end
if nargin < 4; hv = 350:1:1250;  end
if isempty(plot_result); plot_result = 1; end
if isempty(gamma); gamma = 81; end
if isempty(theta); theta = 0; end
if isempty(hv); hv = linspace(350, 1500, 1e4); end
...
% -- Verify that the photon energy is a row vector and theta is a column  vector
if size(theta, 2) > 1;  theta = theta'; end
if size(hv, 1) > 1;     hv = hv'; end
...
for i = 1:Nlyrs
    ...
    lyr_elem{i}     = lyr_cls{i}(1:end-2);              % Corresponding element
    lyr_elem_cls{i} = lyr_cls{i}(end-1:end);          % Corresponding core-level
    ...
    pes_ke{i}   = hv - lyr_be{i} - lyr_WF{i};
    %% 1.2.4 - Determine the IMFP at all kinetic energies for each layer material
    lyr_IMFP{i} = eimfp_tpp2m_mpd(pes_ke{i}, lyr_mat{i});
...
for i = 1:Nlyrs
    ...
    [~, lyr_sigma{i}, lyr_beta{i}] = get_pixsad_props(lyr_elem{i}, lyr_elem_cls{i}, hv);  	% Extract properties from PIXSAD
...
if Nlyrs == 1
    ...
    lyr_ints{1}     = lyr_I0{1} .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta))));
...
elseif Nlyrs == 2
    % -- Extracting the layer intensities
    lyr_ints{1}     = lyr_I0{1} .* (1 - exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta)))));
    lyr_ints{2}     = lyr_I0{2} .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta))));
    % -- Normalising each curve in terms of its relative contribution
...
elseif Nlyrs == 3
    % -- Extracting the layer intensities
    lyr_ints{1}     = lyr_I0{1} .* (1 - exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta)))));
    lyr_ints{2}     = lyr_I0{2} .* (1 - exp(-lyr_thick{2} ./ (lyr_IMFP{2} .* cos(deg2rad(theta))))) .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta))));
    lyr_ints{3}     = lyr_I0{3} .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta)))).* exp(-lyr_thick{2} ./ (lyr_IMFP{2} .* cos(deg2rad(theta))));
    % -- Normalising each curve in terms of its relative contribution
...
elseif Nlyrs == 4
    ...
    lyr_ints{1}     = lyr_I0{1} .* (1 - exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta)))));
    lyr_ints{2}     = lyr_I0{2} .* (1 - exp(-lyr_thick{2} ./ (lyr_IMFP{2} .* cos(deg2rad(theta))))) .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta))));
    lyr_ints{3}     = lyr_I0{3} .* (1 - exp(-lyr_thick{3} ./ (lyr_IMFP{3} .* cos(deg2rad(theta))))) .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta)))).* exp(-lyr_thick{2} ./ (lyr_IMFP{2} .* cos(deg2rad(theta))));
    lyr_ints{4}     = lyr_I0{4} .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta)))).* exp(-lyr_thick{2} ./ (lyr_IMFP{2} .* cos(deg2rad(theta)))).* exp(-lyr_thick{3} ./ (lyr_IMFP{3} .* cos(deg2rad(theta))));
    % -- Normalising each curve in terms of its relative contribution
...
elseif Nlyrs == 5
    ...
    lyr_ints{2}     = lyr_I0{2} .* (1 - exp(-lyr_thick{2} ./ (lyr_IMFP{2} .* cos(deg2rad(theta))))) .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta))));
    lyr_ints{3}     = lyr_I0{3} .* (1 - exp(-lyr_thick{3} ./ (lyr_IMFP{3} .* cos(deg2rad(theta))))) .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta)))).* exp(-lyr_thick{2} ./ (lyr_IMFP{2} .* cos(deg2rad(theta))));
    lyr_ints{4}     = lyr_I0{4} .* (1 - exp(-lyr_thick{4} ./ (lyr_IMFP{4} .* cos(deg2rad(theta))))) .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta)))).* exp(-lyr_thick{2} ./ (lyr_IMFP{2} .* cos(deg2rad(theta)))).* exp(-lyr_thick{3} ./ (lyr_IMFP{3} .* cos(deg2rad(theta))));
    lyr_ints{5}     = lyr_I0{5} .* exp(-lyr_thick{1} ./ (lyr_IMFP{1} .* cos(deg2rad(theta)))).* exp(-lyr_thick{2} ./ (lyr_IMFP{2} .* cos(deg2rad(theta)))).* exp(-lyr_thick{3} ./ (lyr_IMFP{3} .* cos(deg2rad(theta)))).* exp(-lyr_thick{4} ./ (lyr_IMFP{4} .* cos(deg2rad(theta))));
    % -- Normalising each curve in terms of its relative contribution
