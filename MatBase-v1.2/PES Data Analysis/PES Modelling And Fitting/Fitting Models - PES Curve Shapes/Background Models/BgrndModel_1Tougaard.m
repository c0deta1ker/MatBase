function [roi_xdat, roi_ydat, roi_bgrnd] = BgrndModel_1Tougaard(xdat, ydat, LHS, RHS, Win, C)
% [roi_xdat, roi_ydat, roi_bgrnd] = BgrndModel_1Tougaard(xdat, ydat, LHS, RHS, Win, C)
%   Function that determines the best fit to the background of PES (or 1D) 
%   data using a 1-parameter Tougaard background. 
%
%   IN:
...
while cnvgnce > 1e-12
    ...
    for i = 1:nRows-1
        ...
        Fi      = (roi_ydat(i:end)) .* dEi ./ (C + dEi.^2).^2;
