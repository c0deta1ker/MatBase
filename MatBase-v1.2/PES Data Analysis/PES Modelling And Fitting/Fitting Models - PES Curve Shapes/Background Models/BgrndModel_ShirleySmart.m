function [roi_xdat, roi_ydat, roi_bgrnd] = BgrndModel_ShirleySmart(xdat, ydat, LHS, RHS, Win)
% [roi_xdat, roi_ydat, roi_bgrnd] = BgrndModel_ShirleySmart(xdat, ydat, LHS, RHS, Win)
%   Function that determines the best fit to the background of PES (or 1D) 
%   data using a Iterative Smart Shirley background. This has an additional
%   constraint that ensures the Shirley background does not have an
%   intensity higher than the original data.
%
%   IN:
...
%   -   roi_bgrnd:      M×1 (or 1xM) column (or row) vector of the best fit linear background to the ROI data
...
%% - 5 - Determination of the Iterative Shirley Background
...
% - Apply a constraint that ensures the Shirley is not higher than data
constraint  = roi_ydat - roi_bgrnd;
Y_Shift     = min(constraint);
if Y_Shift < 0; roi_bgrnd    = roi_bgrnd + Y_Shift;
else;           roi_bgrnd    = roi_bgrnd;
end
%% -- For Debugging
plot_result = 0;
if plot_result == 1
