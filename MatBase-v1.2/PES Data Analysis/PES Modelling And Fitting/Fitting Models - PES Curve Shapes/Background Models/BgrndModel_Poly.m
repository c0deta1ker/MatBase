function [roi_xdat, roi_ydat, roi_bgrnd] = BgrndModel_Poly(xdat, ydat, LHS, RHS, Win, Order)
% [roi_xdat, roi_ydat, roi_bgrnd] = BgrndModel_Poly(xdat, ydat, LHS, RHS, Win, Order)
%   Function that determines the best fit to the background of PES (or 1D) 
%   data using an n'th order polynomial function. 
%   
%   IN:
...
%   -   Order:          scalar, positive integer of the Polynomial Order
%
%   OUT:
...
%% - 4 - Determination of the Polynomial Background
...
if Order == 0
    ...
    roi_bgrnd       = mean(ydat_bgrnd(:)).*ones(size(roi_ydat));
else
    % -- Fitting the data to a polynomial of the correct order
