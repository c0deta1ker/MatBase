function [roi_xdat, roi_ydat, roi_bgrnd] = BgrndModel_ShirleyOffset(xdat, ydat, LHS, RHS, Win, lambda, delta)
% [roi_xdat, roi_ydat, roi_bgrnd] = BgrndModel_ShirleyOffset(xdat, ydat, LHS, RHS, Win, lambda, delta)
...
% -- Setting the condition for the mixing terms lambda
if      lambda < 0; lambda = 0;
elseif  lambda > 1; lambda = 1;
end
...
%% - 4 - Determination of the Offset Shirley Background
[~, ~, roi_bgrnd_lin]   = BgrndModel_Linear(xdat, ydat, LHS, RHS, Win);
[~, ~, roi_bgrnd_shrly]	= BgrndModel_Shirley(xdat + delta, ydat, LHS, RHS, Win);
