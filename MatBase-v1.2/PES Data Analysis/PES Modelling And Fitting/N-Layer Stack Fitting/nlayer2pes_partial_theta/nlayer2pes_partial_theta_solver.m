function fitStr = nlayer2pes_partial_theta_solver(xpsDat, modelDat, iparams, which_layers, solve_type)
% fitStr = nlayer2pes_partial_theta_solver(xpsDat, modelDat, iparams, which_layers, solve_type)
%   Function that runs an optimisation algorithm to solve for the best fit
%   model sample stack of photoelectron intensities. 
...
XPSObj.which_layers = which_layers;
XPSObj.X            = X;
XPSObj.D   	        = D;
XPSObj.M   	        = M;
...
fitStr.which_layers = which_layers;
...
function M = fit_model(x, theta, XPSObj)
    % -- Define the domain
    X               = theta;
    % -- Extracting the defined model sample
