function [fitStr0, statStr] = nlayer2pes_partial_theta_solver_n_runs(xpsDat, modelDat, iparams, which_layers, solve_type, n_runs)
% [fitStr0, statStr] = nlayer2pes_partial_theta_solver_n_runs(xpsDat, modelDat, iparams, which_layers, solve_type, n_runs)
%   Function that runs 'nlayer2pes_partial_theta_solver()' N amount of independent 
...
for i = statStr.n_runs
    ...
    % --- Executing the fit
    if i == 1;  fitStr{1} = nlayer2pes_partial_theta_solver(xpsDat, modelDat, iparams, which_layers, solve_type);
    else;       fitStr{i} = nlayer2pes_partial_theta_solver(xpsDat, modelDat, new_init_conds, which_layers, solve_type);
    end
