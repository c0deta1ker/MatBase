function fig = nlayer2pes_partial_theta_solver_view_init(xpsDat, modelDat, iparams, which_layers)
% fig = nlayer2pes_partial_theta_solver_view_init(xpsDat, modelDat, iparams, which_layers)
...
%   This function requires that the user define which layers should be used
%   in the fitting to the XPS data. Note, the intensities of the chosen layers
%   are self-normalised to the chosen layers so that it can be directly
%   compared with the XPS data.
%
%   IN:
%   -   xpsDat:         data structure that contains the XPS data.
%           .xdat                   Nx1 column vector of the XPS emission angle (deg.).
...
%           .theta:                 scalar or Nx1 vector of the polar angle between the photoelectron vector relative to electric field vector (i.e. at normal emission: LV (p-pol, E//MP) = 0, LH (s-pol, E⊥MP) = 90) [degree]
...
% -- Ensure correct layers are chosen
which_layers = unique(which_layers);
which_layers(which_layers<1) = [];
which_layers(which_layers>length(modelDat.lyr_mat)) = [];
if isempty(which_layers); which_layers = 1:length(modelDat.lyr_mat); end
...
pes_model       = nlayer_pes_model(lyr_mat, lyr_thick, lyr_ele, lyr_cls, hv, X, phi, P, formalism_xsect, formalism_imfp);
M               = cell2mat(pes_model.lyr_Inorm');
M = M(which_layers,:); M = M ./ sum(M, 1);
% -- Extracting the PES model data on the user-defined grid-size
XX  = theta;
pes_model       = nlayer_pes_model(lyr_mat, lyr_thick, lyr_ele, lyr_cls, hv, XX, phi, P, formalism_xsect, formalism_imfp);
MM              = cell2mat(pes_model.lyr_Inorm');
MM = MM(which_layers,:); MM = MM ./ sum(MM, 1);
...
for i = 1:length(which_layers)
    scatter(X, D(:,i), 'color', lyr_cols{which_layers(i)}, 'markerfacecolor', lyr_cols{which_layers(i)}, 'MarkerFaceAlpha', 0.5, 'markeredgecolor', lyr_cols{which_layers(i)});
end
% -- Plotting the PES model data
for i = 1:length(which_layers)
    plot(XX, MM(:,i), 'k.-', 'color', lyr_cols{which_layers(i)}, 'markerfacecolor', lyr_cols{which_layers(i)}, 'linewidth', 2.5);
    leg_labels{i}   = string(pes_model.lyr_mat{which_layers(i)})+"("+string(pes_model.lyr_cls{which_layers(i)})+")";
    leg_cols{i}     = plot(nan, 'k.-', 'color', lyr_cols{which_layers(i)}, 'markerfacecolor', lyr_cols{which_layers(i)}, 'linewidth', 2.5);
end
...
for i = 1:length(which_layers); bar(X, R(:,i), 'facecolor', lyr_cols{which_layers(i)}); end
grid on;
