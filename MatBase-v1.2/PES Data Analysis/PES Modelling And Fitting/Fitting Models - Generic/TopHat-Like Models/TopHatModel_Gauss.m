function y = TopHatModel_Gauss(x, center, amplitude, width, fwhm)
% y = TopHatModel_Gauss(x, center, amplitude, width, fwhm)
...
%   Used to simulate a top-hat profile that is Gaussian broadened on each
%   side.
%   
%   IN:
...
F2      = exp(-0.5.*((x-center-0.5*width)./sigma).^2); F2 = amplitude .* F2 / max(F2);
F3      = exp(-0.5.*((x-center+0.5*width)./sigma).^2); F3 = amplitude .* F3 / max(F3);
