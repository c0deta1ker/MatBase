function fig = view_nlayer_model02_atmconc(lyr_mat, lyr_thick, lyr_conc)
% fig = view_nlayer_model01_atmconc(lyr_mat, lyr_thick, lyr_conc)
...
xpad = 2.5;
xdat = -0.5:0.01:sum(cell2mat(lyr_thick(1:end-1)))+xpad;
for i = 1:Nlyrs
...
%% 2.2    :   Plotting the atomic concentration with depth
lyr_atmconc = {};
for i = 1:Nlyrs
    for j = 1:length(lyr_conc{i})
        if i == Nlyrs;      lyr_atmconc{i}{j} = TopHatModel(xdat, lyr_x0{i}, lyr_conc{i}{j}, 2*xpad);
        else;               lyr_atmconc{i}{j} = TopHatModel(xdat, lyr_x0{i}, lyr_conc{i}{j}, lyr_thick{i});
        end
    end
end
...
for i = 1:Nlyrs
    for j = 1:length(lyr_conc{i})
        area(xdat, lyr_atmconc{i}{j}, 0, 'facecolor', lyr_cols{i}./j, 'edgecolor', 'none', 'facealpha', 0.5);
        plot(xdat, lyr_atmconc{i}{j}, 'k-', 'color', lyr_cols{i}./j, 'linewidth', 2);
    end
end
% -- Adding text for each material type
for i = 1:Nlyrs
    ...
    for j = 1:length(lyr_conc{i})
        if i == Nlyrs
            text(-y_loc-1., 0.5*lyr_conc{i}{j}, "(Bulk) "+string(lyr_mat{i}{j}),...
            'color', 'k', 'horizontalalignment', 'center', 'verticalalignment', 'middle', 'FontWeight','bold', 'FontSize',9,'Rotation',90);
        else
            text(-y_loc, 0.5*lyr_conc{i}{j}, sprintf("(L%i) %s",i, lyr_mat{i}{j}),...
            'color', 'k', 'horizontalalignment', 'center', 'verticalalignment', 'middle', 'FontWeight','bold', 'FontSize',9,'Rotation',90); 
        end
    end
end
