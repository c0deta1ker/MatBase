function fig = nlayer_model02_view(pes_model)
% fig = nlayer_model02_view(pes_model)
...
if length(pes_model.hv) > 1 && length(pes_model.theta) == 1
    fig{1} = nlayer_model02_view_vs_hv(pes_model);
% -- Single photon energy, but a range of angles
elseif length(pes_model.hv) == 1 && length(pes_model.theta) > 1
    fig{1} = nlayer_model02_view_vs_theta(pes_model);
% -- Range of photon energies and angles
elseif length(pes_model.hv) > 1 && length(pes_model.theta) > 1
    fig{1} = nlayer_model02_view_vs_hv(pes_model);
    fig{2} = nlayer_model02_view_vs_theta(pes_model);
% -- Single values only
else; fig{1} = nlayer_model02_view_vs_hv(pes_model);
end