function fig = view_nlayer_model01_atmconc(lyr_mat, lyr_thick, lyr_fwhm)
% fig = view_nlayer_model01_atmconc(lyr_mat, lyr_thick, lyr_fwhm)
...
for i = 1:Nlyrs
    area(xdat, lyr_atmconc{i}, 0, 'facecolor', lyr_cols{i}, 'edgecolor', 'none', 'facealpha', 0.5);
    plot(xdat, lyr_atmconc{i}, 'k-', 'color', lyr_cols{i}, 'linewidth', 2);
end
% -- Adding text for each material type
for i = 1:Nlyrs
    if i == Nlyrs
        text(lyr_x0{i}-0.5*xpad, 120, sprintf("(Bulk) \n %s", lyr_mat{i}),...
        'color', 'k', 'horizontalalignment', 'center', 'verticalalignment', 'middle', 'FontWeight','bold', 'FontSize',9);
    else
        text(lyr_x0{i}, 120, sprintf("(L%i)\n %s \n %.1f nm",i, lyr_mat{i}, lyr_thick{i}),...
        'color', 'k', 'horizontalalignment', 'center', 'verticalalignment', 'middle', 'FontWeight','bold', 'FontSize',9); 
    end
end
% -- Formatting the axes
