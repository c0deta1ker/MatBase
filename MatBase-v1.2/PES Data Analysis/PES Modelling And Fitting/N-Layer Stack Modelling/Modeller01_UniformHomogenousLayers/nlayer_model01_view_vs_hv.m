function fig = nlayer_model01_view_vs_hv(pes_model)
...
for i = 1:pes_model.Nlyrs
    ...
    if length(pes_model.theta) == 1
        plot(pes_model.hv, pes_model.lyr_Inorm{i}, 'k.-', 'color', lyr_cols{i}, 'markerfacecolor', lyr_cols{i}, 'linewidth', 2.5);
    % -- If a range of angles are to be plotted
        ...
        for j = 1:length(pes_model.theta)
            plot(pes_model.hv, pes_model.lyr_Inorm{i}(:,j), 'k.-', 'color', colors_p(j,:), 'markerfacecolor', colors_p(j,:), 'linewidth', 2.5);
        end
    end
