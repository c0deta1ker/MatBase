function xsect_gamma = xsect_gamma_trzh2018(element, corelevel, hv, plot_results)
...
if size(hv, 2) > 1;         hv = hv'; end
xsect_gamma = interp1(hv_data, xsect_data, hv, 'linear');
%% -- Plot for debugging
if plot_results == 1
    nCL     = size(xsect_gamma, 2);
    cols    = lines(nCL);
    ...
    title('Trzh. & Yarz. 2018 - Photoionisation Parameter (gamma)');
